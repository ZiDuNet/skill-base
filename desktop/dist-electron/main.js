...
async function loadDesktopCli(cliLibRoot) {
  const importCli = (subpath) => import(pathToFileURL(path.join(cliLibRoot, subpath)).href);
  const [configMod, authMod, apiMod, ideMod, installsMod, extractMod, updateHelpersMod] = await Promise.all([
    importCli("config.js"),
    importCli("auth.js"),
    importCli("api.js"),
    importCli("ide.js"),
...
function registerDesktopHandlers(cli2, cliLibRoot, deps) {
  function getSavedProjectRoot() {
    const saved = cli2.loadSavedConfig();
  ...
  function saveProjectRoot(root) {
    cli2.saveConfig({ desktopProjectRoot: root });
  ...
  function buildProjectTargets(cwd) {
    const { IDE_CONFIGS, findProjectRoot } = cli2;
  ...
  const handlers = {
    "config:get": async () => {
      const { baseUrl } = cli2.getConfig();
      const cred = cli2.loadCredentials();
      return {
        baseUrl,
        username: (cred == null ? void 0 : cred.username) || null,
        hasToken: Boolean(cred == null ? void 0 : cred.token),
    ...
    "config:setServer": async (url) => {
      ...
      cli2.saveConfig({ baseUrl: normalized });
    ...
    "skills:openWebPage": async (payload) => {
      ...
      const skillId = String((payload == null ? void 0 : payload.skillId) || "").trim();
      if (!skillId) throw new Error("skillId required");
      let url;
      if ((payload == null ? void 0 : payload.page) === "diff") {
        ...
        const version = String((payload == null ? void 0 : payload.version) || "").trim();
        url = version ? `${base}/skills/${encodeURIComponent(skillId)}?version=${encodeURIComponent(version)}` : `${base}/skills/${encodeURIComponent(skillId)}`;
      }
    ...
    "skills:getInstalled": async () => {
      var _a, _b, _c;
      cli2.pruneAllMissingInstalls();
      const rows = cli2.listInstalledSkills();
      const client = cli2.createClient();
      const enriched = [];
      for (const row of rows) {
        ...
        const firstPath = (_a = row.installs[0]) == null ? void 0 : _a.installPath;
        const meta = firstPath ? await readLocalSkillMeta(firstPath) : { name: row.skillId, description: "" };
        enriched.push({
          skillId: row.skillId,
          name: meta.name,
          description: meta.description,
          version: ((_b = row.installs[0]) == null ? void 0 : _b.version) || "",
          latest: (remote == null ? void 0 : remote.latest_version) || ((_c = row.installs[0]) == null ? void 0 : _c.version) || "",
          author: (remote == null ? void 0 : remote.owner_username) || "",
    ...
    "skills:install": async (payload) => {
      const skillId = String((payload == null ? void 0 : payload.skillId) || "").trim();
      const version = (payload == null ? void 0 : payload.version) || "latest";
      const installProjectRoot = typeof (payload == null ? void 0 : payload.projectRoot) === "string" ? payload.projectRoot.trim() : "";
      const overwrite = Boolean(payload == null ? void 0 : payload.overwrite);
      const acceptNested = Boolean(payload == null ? void 0 : payload.acceptNested);
      const rawTargets = Array.isArray(payload == null ? void 0 : payload.targets) ? payload.targets : [];
      if (!skillId) {
      ...
      for (const spec of resolved) {
        const insideIde = cli2.detectInsideIdeDir(spec.nestedCheckDir);
        if (insideIde && !acceptNested) {
          return {
            ok: false,
            code: "NESTED_IDE_PATH",
            detail: spec.kind === "custom" ? `所选目录已在 ${insideIde.name} 的 skill 路径内，若仍要安装请勾选确认嵌套。` : spec.kind === "project" ? `项目目录已在 ${insideIde.name} 的 skill 路径内，若仍要安装请勾选确认嵌套。` : `当前目录已在 ${insideIde.name} 的 skill 路径内，若仍要安装请勾选确认嵌套。`,
            ideId: insideIde.id
          };
        }
      }
      ...
      for (const spec of resolved) {
        ...
        const result = await cli2.downloadAndExtract(skillId, version, spec.targetDir);
        actualVersion = result.version;
        const installPath = path.join(result.targetDir, skillId);
        cli2.rememberSkillInstall({
    ...
    "skills:update": async (payload) => {
      const skillId = String((payload == null ? void 0 : payload.skillId) || "").trim();
      const version = String((payload == null ? void 0 : payload.version) || "").trim();
      let installPaths = Array.isArray(payload == null ? void 0 : payload.installPaths) ? payload.installPaths.filter((p) => typeof p === "string") : null;
      if (!skillId || !version) throw new Error("skillId and version required");
      const options = {};
      const installs = cli2.buildTargetInstalls(skillId, options);
      if (installs.length === 0) {
        throw new Error(`本地没有记录到 ${skillId} 的安装目录`);
      }
      if (!installPaths || installPaths.length === 0) {
        const implicit = cli2.resolveImplicitSelectedInstalls(installs, options);
        if (!implicit) {
          return {
            ok: false,
            code: "PICK_PATHS",
      ...
      for (const install of selected) {
        const result = await cli2.downloadAndExtract(
          skillId,
          version,
...
function getCliLibRoot() {
  ...
  return path.join(__dirname$1, "../../cli/lib");
}
let projectRoot = "";
let mainWindow = null;
let cli;
