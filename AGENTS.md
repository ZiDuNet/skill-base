...
├── desktop/      # Desktop 客户端 (Electron，legacy — 见 desktop/DEPRECATED.md)
├── desktop-tauri/ # Desktop 客户端 (Tauri 2，推荐 — Node bridge + 共用 desktop/src UI)
├── src/          # 服务端源码 (Fastify)
...
- Electron：`ipcMain.handle` 直接调用 handlers
